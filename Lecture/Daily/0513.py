...
# 시스템 명령어
# environ : 환경변수를 보여줌
# getcwd  : 현재 작업 위치를 보여줌
# 주로 파이썬에서 os 명령어를 사용하고 싶을 때 사용
"""
import os
...
# 프로그램이 실행을 하게되면 프로세스로 이름이 바뀌게 됨
# 각각의 프로세스를 완전히 독립된 상태라 서로 정보(값) 을 주고 받을 수 없음
# 여기서 네트워크 프로그램이 문제가 되는 데 본래 프로그램은 한 가지 기능만 해야함
# (데이터를 받기만 하거나 보내기만 하거나)

# 멀티 프로세싱
# 여러개의 프로세스가 한 가지 목적으로 움직이는 것

# IPC = 프로세스 간 통신
# OS 는 데이터를 주고 받을 수 있기에 OS 가 중간에서 프로세스와 프로세스 사이에서 데이터를 주고받는 것
 

# 멀티 스레딩
# 전체적인 작업 시간은 느리지만 응답성이 빨라짐