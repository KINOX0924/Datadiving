...
# //FIXME [1] 관리자 모듈 수정 작업 전에 이것 먼저 수정 - 패턴 설정 필요 //TODO [1] 수정 완료
def checkPhoneNumber(phone_number) :
    phonenumber_pattern = r"^\d{3}-\d{3,4}-\d{4}\b"
    phonenumber = re.finditer(phonenumber_pattern , phone_number)
    
    search_phonenumber = ""
    
    for value in phonenumber :
        search_phonenumber = value.group()
